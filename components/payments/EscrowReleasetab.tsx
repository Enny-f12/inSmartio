...
import { releaseEscrow } from "@/lib/api/paymentApi";
...
export default function EscrowReleasesTab() {
  ...
  const [releasing, setReleasing] = useState<Set<string>>(new Set()); // tracks in-flight releases
  ...
  // ── Release single escrow ─────────────────────────────
  const handleRelease = async (id: string) => {
    setReleasing((prev) => new Set(prev).add(id));
    try {
      await releaseEscrow(id);
      toast.success(`Escrow ${id} released successfully`);
      dispatch(fetchEscrows()); // refresh list
    } catch {
      toast.error(`Failed to release escrow ${id}`);
    } finally {
  ...
  const handleReleaseSelected = async () => {
    ...
    await Promise.allSettled(ids.map((id) => handleRelease(id)));
    setSelected(new Set());
  };

  // ── Release all visible escrows ───────────────────────
      ...
      <div style={{ borderRadius: "16px", border: "1px solid var(--color-border)",
        ...
        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--color-border)",
          ...
          <p style={{ fontSize: "14px", fontWeight: 600, color: "var(--color-text-main)", margin: 0 }}>
            ...
            {filtered.length > 0 && (
              <span style={{ marginLeft: 8, fontSize: "12px", fontWeight: 600, padding: "2px 8px",
                borderRadius: 999, backgroundColor: "#FEF3C7", color: "#B45309" }}>
                {filtered.length}
              </span>
            )}
          </p>
          <div style={{ position: "relative", minWidth: 200 }}>
            ...
            <input type="text" placeholder="Search job, client, expert…" value={search}
              ...
              style={{ paddingLeft: "36px", paddingRight: "12px", paddingTop: "8px",
                paddingBottom: "8px", borderRadius: "10px", border: "1px solid var(--color-border)",
                fontSize: "13px", outline: "none", backgroundColor: "var(--color-background)",
                color: "var(--color-text-main)", width: "100%", boxSizing: "border-box" }} />
          </div>
        </div>
                  ...
                  <tr style={{ borderBottom: "1px solid var(--color-border)",
                    ...
                    <th style={{ padding: "12px 16px 12px 20px", width: 32 }}>
                      <input type="checkbox"
                        ...
                        onChange={() => selected.size === paginated.length
                          ? setSelected(new Set())
                    ...
                    {["Job ID", "Client", "Expert", "Amount", "Days Left", "Status", "Actions"].map((h) => (
                      <th key={h} style={{ textAlign: "left", padding: "12px 20px",
                        ...
                        letterSpacing: "0.05em", color: "var(--color-text-muted)" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? (
                    ...
                    const isReleasing = releasing.has(e.id);
                    return (
                      <tr key={e.id} style={{ borderBottom: "1px solid var(--color-border)",
                        backgroundColor: selected.has(e.id) ? "#F0FDF4" : undefined }}>
                        <td style={{ padding: "15px 16px 15px 20px" }}>
                          <input type="checkbox" checked={selected.has(e.id)}
                            onChange={() => toggleSelect(e.id)}
                            style={{ accentColor: "#16a34a", width: 14, height: 14 }} />
                        </td>
                        <td style={{ padding: "15px 20px", fontSize: "13px", fontWeight: 600,
                          color: "var(--color-text-main)", fontFamily: "monospace" }}>{e.id}</td>
                        <td style={{ padding: "15px 20px", fontSize: "13px",
                          color: "var(--color-text-main)" }}>{e.userId}</td>
                        <td style={{ padding: "15px 20px", fontSize: "13px",
                          color: "var(--color-text-main)" }}>{e.expertId ?? "—"}</td>
                        <td style={{ padding: "15px 20px", fontSize: "13px", fontWeight: 600,
                          color: "var(--color-text-main)", whiteSpace: "nowrap" }}>
                          ₦{Number(e.amount).toLocaleString()}
                        </td>
                        <td style={{ padding: "15px 20px", fontSize: "13px", fontWeight: 600,
                          color: urgent ? "#dc2626" : "#d97706", whiteSpace: "nowrap" }}>
                        ...
                        <td style={{ padding: "15px 20px" }}>
                          <button onClick={() => handleRelease(e.id)} disabled={isReleasing}
                            style={{ fontSize: "12px", fontWeight: 600, padding: "5px 14px",
                            ...
                            {isReleasing
                              ? <><Loader2 size={11} className="animate-spin" /> Releasing…</>
                              : "Release"}
                          </button>
                        </td>
                      </tr>
            ...
            <div className="escrow-cards" style={{ backgroundColor: "var(--color-background)" }}>
              {paginated.length === 0
                ? <p style={{ textAlign: "center", padding: "40px", fontSize: "13px",
                    color: "var(--color-text-muted)" }}>No pending escrows.</p>
                : paginated.map((e) => {
                  const urgent     = e.daysLeft <= 1;
                  const isReleasing = releasing.has(e.id);
                  return (
                    <div key={e.id} onClick={() => toggleSelect(e.id)}
                      style={{ padding: "14px 16px", borderRadius: "12px",
                        border: `1px solid ${selected.has(e.id) ? "#16a34a" : "var(--color-border)"}`,
                        backgroundColor: selected.has(e.id) ? "#F0FDF4" : "#fff" }}>
                      <div style={{ display: "flex", justifyContent: "space-between",
                        gap: 8, marginBottom: 6 }}>
                        <div>
                          <p style={{ fontSize: "12px", fontFamily: "monospace", fontWeight: 600,
                            color: "var(--color-text-main)", marginBottom: 2 }}>{e.id}</p>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <p style={{ fontSize: "13px", fontWeight: 600,
                            color: "var(--color-text-main)", marginBottom: 4 }}>
                            ₦{Number(e.amount).toLocaleString()}
                          </p>
                      ...
                      <button disabled={isReleasing}
                        onClick={(ev) => { ev.stopPropagation(); handleRelease(e.id); }}
                        style={{ width: "100%", marginTop: 8, padding: "8px", borderRadius: "8px",
                          border: "1px solid #16a34a", backgroundColor: "#fff", color: "#16a34a",
