...
function StatusPill({ status }: { status?: string }) {
  ...
  return (
    <span style={{ fontSize: "11px", fontWeight: 600, padding: "3px 10px", borderRadius: "20px", whiteSpace: "nowrap", background: bg, color, border }}>
      {status ?? "—"}
    </span>
  );
}
...
type EnrichedEscrow = ApiEscrow & { daysLeft: number };

function EscrowDetailModal({ escrow, onClose }: { escrow: EnrichedEscrow; onClose: () => void }) {
  const rec = escrow as Record<string, unknown>;

  return (
    <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "16px" }}>
      <div style={{ backgroundColor: "#fff", borderRadius: "16px", width: "100%", maxWidth: "480px", boxShadow: "0 16px 48px rgba(0,0,0,0.18)", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid #E5E7EB" }}>
          <p style={{ fontSize: "15px", fontWeight: 700, color: "#111827", margin: 0 }}>Escrow Detail</p>
          <button onClick={onClose} style={{ padding: "4px", borderRadius: "8px", border: "none", background: "none", cursor: "pointer", color: "#9CA3AF", display: "flex" }}>
            <X size={18} />
          </button>
        </div>

        <div style={{ padding: "20px", maxHeight: "70vh", overflowY: "auto" }}>
          <p style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#6B7280", marginBottom: "14px" }}>Escrow Information</p>
          <div style={{ backgroundColor: "#F9FAFB", borderRadius: "12px", padding: "16px", marginBottom: "16px" }}>
            <DetailRow label="Escrow ID:"      value={escrow.id} />
            <DetailRow label="Job ID:"         value={String(rec.resourceId ?? rec.jobId ?? "—")} />
            <DetailRow label="Provider:"       value={String(escrow.provider ?? "—")} />
            <DetailRow label="Reference:"      value={String(rec.reference ?? "—")} />
            <DetailRow label="Created:"        value={escrow.createdAt ? new Date(escrow.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"} />
            <DetailRow label="Days Left:"      value={<span style={{ fontWeight: 600, color: escrow.daysLeft <= 1 ? "#dc2626" : "#d97706" }}>{escrow.daysLeft} {escrow.daysLeft === 1 ? "day" : "days"}</span>} />
            <DetailRow label="Escrow Status:"  value={<StatusPill status={escrow.escrowStatus} />} />
            <DetailRow label="Payment Status:" value={<StatusPill status={escrow.status} />} />
          </div>
          ...
          <div style={{ backgroundColor: "#F9FAFB", borderRadius: "12px", padding: "16px" }}>
            <DetailRow label="Amount:"   value={<span style={{ fontWeight: 600, fontSize: "15px", color: "#111827" }}>₦{Number(escrow.amount).toLocaleString()}</span>} />
            <DetailRow label="Currency:" value={String(rec.currency ?? "NGN")} />
            <DetailRow label="Purpose:"  value={String(rec.purpose ?? "—")} />
            <DetailRow label="Released:" value={rec.releasedAt ? new Date(String(rec.releasedAt)).toLocaleDateString("en-GB") : "Not yet released"} />
          </div>
        </div>

        <div style={{ padding: "14px 20px", borderTop: "1px solid #E5E7EB", display: "flex", justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ padding: "10px 24px", borderRadius: "10px", border: "1px solid #E5E7EB", backgroundColor: "#fff", fontSize: "13px", fontWeight: 500, cursor: "pointer", color: "#374151" }}>
            Close
          </button>
        </div>
      </div>
...
export default function EscrowReleasesTab() {
  ...
  const [viewing, setViewing] = useState<EnrichedEscrow | null>(null);

  // ── useRef fixes react-hooks/purity (Date.now is impure) ──
  ...
  const data: EnrichedEscrow[] = useMemo(() => escrows.map((e) => {
    const rec      = e as Record<string, unknown>;
    const daysLeft = typeof rec.daysLeft === "number"
      ? rec.daysLeft
      : Math.max(0, Math.round((new Date(e.createdAt).getTime() + 7 * 86400000 - nowRef.current) / 86400000));
    return { ...e, daysLeft };
  }), [escrows]);
  ...
  const TH: React.CSSProperties = { textAlign: "left", padding: "12px 20px", fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--color-text-muted)" };
  const TD: React.CSSProperties = { padding: "15px 20px", fontSize: "13px", color: "var(--color-text-main)", borderBottom: "1px solid var(--color-border)" };
  ...
  return (
    <>
                      ...
                      <tr key={e.id} style={{ borderBottom: "1px solid var(--color-border)" }}
                        ...
                        <td style={TD}>
                          <button onClick={() => setViewing(e)} title="View details"
                            style={{ padding: "6px", borderRadius: "8px", border: "none", background: "none", cursor: "pointer", color: "#9CA3AF", display: "flex" }}>
                  ...
                  return (
                    <div key={e.id} style={{ padding: "14px 16px", borderRadius: "12px", border: "1px solid var(--color-border)", backgroundColor: "#fff", display: "flex", alignItems: "center", gap: "12px" }}>
                      ...
                      <button onClick={() => setViewing(e)}
                        style={{ padding: "8px", borderRadius: "8px", border: "1px solid var(--color-border)", background: "none", cursor: "pointer", color: "#9CA3AF", flexShrink: 0, display: "flex" }}>
      ...
      {viewing && <EscrowDetailModal escrow={viewing} onClose={() => setViewing(null)} />}
    </>
  );
}