...
export function DataTable<T extends { id: number | string }>({
  ...
  const filtered = data.filter((row) => {
    const matchSearch = searchKey
      ...
      : true;
    const matchFilter =
      !filterKey || !filter || filter === filterOptions?.[0] ||
    ...
    <div className="rounded-2xl border border-border bg-surface overflow-hidden">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-border">
        ...
        <div className="flex items-center gap-3">
          ...
          {filterOptions && (
            <FilterDropdown value={filter} options={filterOptions} onChange={handleFilter} />
          )}
          ...
          {onExport && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-medium border border-border bg-surface text-text-muted hover:bg-background transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {exportLoading
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-background">
              {columns.map((col) => (
                <th key={String(col.key)} className={`text-left px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-text-muted ${col.className ?? ""}`}>
                  {col.header}
                </th>
              ))}
            </tr>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button key={p} onClick={() => setPage(p)}
              className={`w-8 h-8 rounded-lg text-[12px] font-medium transition-all ${p === page ? "btn-primary" : "border border-border bg-surface text-text-muted hover:bg-background"}`}>
              {p}
            </button>
          ))}
