...
function ChangePasswordModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    if (form.newPassword.length < 8) return;
    if (form.newPassword !== form.confirm) return;
    setLoading(true);
    try {
  ...
  const match = form.confirm.length > 0
...
function NotificationPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0,
      width: "340px", maxWidth: "calc(100vw - 32px)", borderRadius: "16px",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        ...
        <div style={{ display: "flex", gap: "4px" }}>
          {unreadCount > 0 && (
            <button onClick={() => dispatch(markAllReadThunk())} title="Mark all read"
      ...
      <div style={{ maxHeight: "380px", overflowY: "auto" }}>
        {listStatus === "loading" && (
          <p style={{ textAlign: "center", padding: "32px", fontSize: "13px", color: "#9CA3AF" }}>Loading…</p>
        )}
        {listStatus === "failed" && (
          <p style={{ textAlign: "center", padding: "32px", fontSize: "13px", color: "#EF4444" }}>Failed to load.</p>
        )}
          ...
          return (
            <div key={n.id} style={{ display: "flex", gap: "10px", padding: "12px 16px",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {n.title && (
                  <p style={{ fontSize: "13px", fontWeight: 600, color: "#111827",
                    margin: "0 0 2px", lineHeight: 1.4 }}>{n.title}</p>
                )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", flexShrink: 0 }}>
                {!read && (
                  <button onClick={() => dispatch(markReadThunk(n.id))} title="Mark read"
                    style={{ padding: "4px", borderRadius: "6px", border: "none",
                      background: "none", cursor: "pointer", color: "#9CA3AF" }}>
                    <CheckCheck size={13} />
                  </button>
                )}
                <button onClick={() => dispatch(deleteNotificationThunk(n.id))} title="Delete"
                  style={{ padding: "4px", borderRadius: "6px", border: "none",
...
function ProfileDropdown({
  ...
  const item = (icon: React.ReactNode, label: string, onClick: () => void, danger = false) => (
    <button onClick={() => { onClose(); onClick(); }}
      style={{ width: "100%", display: "flex", alignItems: "center", gap: "10px",
        ...
        cursor: "pointer", fontSize: "13px", color: danger ? "#EF4444" : "#374151",
  ...
  return (
    <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0,
      width: "230px", borderRadius: "16px", border: "1px solid #E5E7EB",
      backgroundColor: "#ffffff", boxShadow: "0 8px 32px rgba(0,0,0,0.12)",
          ...
          <div ref={notifRef} style={{ position: "relative" }}>
            <button onClick={() => { setShowNotifs((v) => !v); setShowProfile(false); }}
              ...
              style={{ position: "relative", padding: "8px", borderRadius: "10px",
                border: "none", background: showNotifs ? "#F9FAFB" : "none", cursor: "pointer",
          ...
          <div ref={profileRef} style={{ position: "relative" }}>
            <button onClick={() => { setShowProfile((v) => !v); setShowNotifs(false); }}
              style={{ display: "flex", alignItems: "center", gap: "8px",
                padding: "5px 8px", borderRadius: "10px", border: "none",
                background: showProfile ? "#F9FAFB" : "none", cursor: "pointer",
