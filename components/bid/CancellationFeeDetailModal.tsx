...
import { X, Image as ImageIcon, Video, CheckCircle, AlertTriangle, Loader2, Flag } from "lucide-react";
import {
        ...
        {detailLoading || !fee ? (
          ...
          <div className="p-5 space-y-5">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                ...
                {fee.evidenceUrls.length > 0 && (
                  <div className="flex items-center flex-wrap gap-2 mt-2">
                    <span className="text-xs text-text-muted">Evidence Uploaded:</span>
              ...
              <div className="border border-border rounded-xl p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-text-muted">
                    Calculation: {fmt(fee.originalAmount)} × 25% = {fmt(Math.round(fee.originalAmount * 0.25))}
                  </span>
                  <span className="font-medium">{fmt(Math.round(fee.originalAmount * 0.25))}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-muted">Fee Cap Applied (Maximum ₦10,000)</span>
                  <span className="font-medium text-amber-600">Maximum (₦10,000)</span>
                </div>
                ...
                <div className="border-t border-border pt-2 space-y-1 mt-1">
            ...
            <section>
              ...
              {fee.adminVerified ? (
                <div className="flex items-center gap-2 text-green-600 border border-border rounded-xl p-4">
                ...
                <div className="border border-border rounded-xl p-4 space-y-4">
                  ...
                  {/* Adjusted amount — always visible per spec */}
                  <div>
                    ...
                    <input
                      ...
                      disabled={verifyMode !== "adjust"}
                      className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-40 disabled:bg-gray-50"
                    />
                  </div>

                  {/* Reason — always visible per spec */}
                  <div>
                    <label className="text-xs text-text-muted block mb-1">
                    ...
                    <textarea
                      ...
                      disabled={verifyMode !== "adjust"}
                      rows={3}
                      className="w-full px-3 py-2 border border-border rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-40 disabled:bg-gray-50"
                    />
                  </div>
                  ...
                  {/* ── Action buttons — all three always visible per spec ── */}
                  <div className="flex items-center gap-2 flex-wrap pt-1">
                    <button
                      onClick={handleConfirm}
                      disabled={actionLoading || verifyMode !== "correct"}
                      className="btn-primary flex items-center gap-2 text-sm disabled:opacity-40"
                    >
                      {actionLoading && verifyMode === "correct" && (
                    ...
                    <button
                      ...
                      disabled={actionLoading || verifyMode !== "adjust" || !adjustedAmount || !adjustReason}
                      className="px-4 py-2 border border-border rounded-lg text-sm text-text-main hover:bg-gray-50 transition-colors disabled:opacity-40 flex items-center gap-2"
                    >
                      {actionLoading && verifyMode === "adjust" && (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      )}
                      Adjust Fee
                    </button>
                    ...
                    <button
                      className="px-4 py-2 border border-red-200 bg-red-50 text-red-700 rounded-lg text-sm font-medium hover:bg-red-100 transition-colors flex items-center gap-1.5"
                    >
