...
// Per spec: status is just Active or Reject
...
// Flag icon color
const flagIconColor = (priority?: string) => {
...
function KPICard({
  label, value, delta, alert, alertMsg,
}: {
  label: string; value: string | number; delta: number; alert?: boolean; alertMsg?: string;
}) {
  const up = delta >= 0;
  return (
    <div className={`bg-surface rounded-xl border p-4 flex flex-col gap-1 relative ${
      ...
      <span className="text-xs text-text-muted font-medium uppercase tracking-wide pr-5">
        {label}
      </span>
...
export default function BidManagementPage() {
  ...
  const topFlag = (bid: Bid) => bid.flags.find((f) => f.status === "open") ?? null;
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      ...
      <div className="flex-1 overflow-y-auto bg-background p-4 sm:p-6 space-y-4">
          ...
          {kpiLoading || !kpi ? (
            ...
            <>
              ...
              <KPICard
                ...
                alertMsg={kpi.cancellationFeeRate > 10
        ...
        <div className="bg-surface rounded-xl border border-border p-4 space-y-3">
          {/* Process stage filters — matches spec exactly */}
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {STEP_FILTERS.map((f) => (
                <button
                  key={String(f.value)}
                  onClick={() => dispatch(setFilters({ step: f.value }))}
                ...
                {listLoading ? (
                  ...
                  bids.map((bid) => {
                    ...
                    return (
                      <tr
                        key={bid.id}
                        className="border-b border-border hover:bg-gray-50 transition-colors text-xs"
                      >
                        {/* ID */}
                        ...
                        <td className="px-4 py-3 text-right">
                          {bid.cancellationFee
                            ? <span className="font-medium text-red-600">{fmt(bid.cancellationFee)}</span>
                        ...
                        <td className="px-4 py-3 text-center">
                          <Flag
                            className={`w-4 h-4 mx-auto ${flagIconColor(flag?.priority)}`}
