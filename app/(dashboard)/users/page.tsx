...
const toUser = (u: ApiUser, avatarSeed: number): User => ({
  ...
  services: u.services,
  // bankDetails comes directly from API for TAS
  bankDetails: u.bankDetails,
  document: u.document,
  paymentModel: u.paymentModel,
  location: u.location,
  // TAS uses dateOfBirth from API, map to dob for UI
  dob: (u as unknown as Record<string, unknown>).dateOfBirth as string ?? u.dob,
  referral: u.referral,
  // account is legacy field, but also check bankDetails for TAS
...
export default function UsersPage() {
  ...
  // Derive detail view from Redux state — no setState in effect
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
      ...
      <main className="users-main" style={{ flex: 1 }}>
        ...
        {listStatus === "succeeded" && (
          <DataTable data={users} columns={columns} filterOptions={FILTER_OPTIONS} filterKey="type" searchKey="name" searchPlaceholder="Search name..." pageSize={10} onExport={() => {}} />
        )}
      </main>
